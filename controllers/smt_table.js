...
function getMCTable(callback) {
  db.execute("SELECT * FROM MÉDECINE_TRAVAIL", (err, results) => {
...
function deleteRows(selectedData, callback) {
  ...
  selectedData.forEach((ele) => {
    db.execute(
      "DELETE FROM MÉDECINE_TRAVAIL WHERE ID = ?;",
      [ele.ID],
      (err, results) => {
        if (err) console.log(err);
  ...
  if (table !== null) {
    ...
    for (var i = 0; i < res.length; i++) {
      ...
      if (!(typeof modifItems.ID == "string")) {
        db.query(
          "UPDATE MÉDECINE_TRAVAIL SET " + modifItems.col + "= ? WHERE ID = ?;",
          [
            // modifItems.col,
            modifItems.value,
        ...
        if (typeof modifItems.ACTES !== "undefined") ACTES = modifItems.ACTES;
        ...
        if (typeof modifItems.TEL !== "undefined") TEL = modifItems.TEL;
        db.query(
          "INSERT INTO MÉDECINE_TRAVAIL(ID,ACTES,STRUCTURES,ADRESSE,TEL) VALUES(?,?,?,?,?);",
          [ID, ACTES, STRUCTURES, ADRESSE, TEL],
          (err, results) => {
            if (err) console.log(err);
