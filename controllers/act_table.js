...
function getACTable(callback) {
  db.execute(
    "SELECT ACT.ID, ACT.CODE, ACT.DÉSIGNATION, ACT.MONTANT_GLOBAL_TTC, STP.STRUCTURES " +
      "FROM ACT, STRUCTURES_TIERS_PAYANT as STP " +
...
function setACTable(table, callback) {
  if (table !== null) {
    ...
    for (var i = 0; i < res.length; i++) {
      ...
      if (!(typeof modifItems.ID == "string")) {
        db.query(
          "UPDATE ACT SET " + modifItems.col + "= ? WHERE ID = ?;",
          [
            // modifItems.col,
            modifItems.value,
        ...
        if (typeof modifItems.DÉSIGNATION !== "undefined")
          DÉSIGNATION = modifItems.DÉSIGNATION;
        if (typeof modifItems.MONTANT_GLOBAL_TTC !== "undefined")
          MONTANT_GLOBAL_TTC = modifItems.MONTANT_GLOBAL_TTC;
        db.query(
          "INSERT INTO ACT(ID,CODE,DÉSIGNATION,MONTANT_GLOBAL_TTC) VALUES(?,?,?,?);",
          [ID, CODE, DÉSIGNATION, MONTANT_GLOBAL_TTC],
          (err, results) => {
            if (err) console.log(err);
